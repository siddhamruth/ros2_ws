  ...
  my_robot_interfaces__msg__HardwareStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    my_robot_interfaces__msg__HardwareStatus * data =
      (my_robot_interfaces__msg__HardwareStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!my_robot_interfaces__msg__HardwareStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          my_robot_interfaces__msg__HardwareStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
