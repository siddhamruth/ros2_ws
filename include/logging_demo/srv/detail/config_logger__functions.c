  ...
  logging_demo__srv__ConfigLogger_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    logging_demo__srv__ConfigLogger_Request * data =
      (logging_demo__srv__ConfigLogger_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!logging_demo__srv__ConfigLogger_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          logging_demo__srv__ConfigLogger_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  logging_demo__srv__ConfigLogger_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    logging_demo__srv__ConfigLogger_Response * data =
      (logging_demo__srv__ConfigLogger_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!logging_demo__srv__ConfigLogger_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          logging_demo__srv__ConfigLogger_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
