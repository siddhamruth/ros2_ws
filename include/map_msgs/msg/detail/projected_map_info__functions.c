  ...
  map_msgs__msg__ProjectedMapInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    map_msgs__msg__ProjectedMapInfo * data =
      (map_msgs__msg__ProjectedMapInfo *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!map_msgs__msg__ProjectedMapInfo__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          map_msgs__msg__ProjectedMapInfo__fini(&data[i]);
        }
        free(data);
        return false;
      }
