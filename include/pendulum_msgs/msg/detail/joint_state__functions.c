  ...
  pendulum_msgs__msg__JointState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    pendulum_msgs__msg__JointState * data =
      (pendulum_msgs__msg__JointState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pendulum_msgs__msg__JointState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          pendulum_msgs__msg__JointState__fini(&data[i]);
        }
        free(data);
        return false;
      }
